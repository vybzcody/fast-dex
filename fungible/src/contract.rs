...
use fungible::{
    FungibleOperation, FungibleResponse, FungibleTokenAbi, InitialState, Message, Parameters, Account,
};
use linera_sdk::{
    linera_base_types::{Account as BaseAccount, AccountOwner, Amount, WithContractAbi},
    views::{RootView, View},
    ...
    async fn execute_operation(&mut self, operation: Self::Operation) -> Self::Response {
        match operation {
            ...
            FungibleOperation::Transfer {
                ...
                let target = BaseAccount {
                    chain_id: target_account.chain_id,
                    owner: target_account.owner,
                };
                self.finish_transfer_to_account(amount, target, owner)
                    .await;
                FungibleResponse::Ok
            }
            ...
            FungibleOperation::Claim {
                ...
                self.claim(source, amount, target).await;
                FungibleResponse::Ok
            }
        }
    }
...
impl FungibleTokenContract {
    async fn claim(&mut self, source_account: BaseAccount, amount: Amount, target_account: BaseAccount) {
        if source_account.chain_id == self.runtime.chain_id() {
            ...
            let message = Message::Withdraw {
                owner: source_account.owner,
                amount,
                target_account: Account { // Constructing ABI Account for message
                     chain_id: target_account.chain_id,
                     owner: target_account.owner,
                },
            };
